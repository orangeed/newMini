...
export default {
  ...
  methods: {
    ...
    onMixinDatacomPropsChange(needReset) {
...
<template>
  <view :id="`ChartBoxId${cid}`" class="chartsview">
    ...
    <block v-if="echarts">
      <view
        v-show="showchart" :id="`EC${cid}`" :style="{ background }" style="width: 100%;height: 100%;"
        :data-directory="directory" :prop="echartsOpts" :change:prop="rdcharts.ecinit" :resize="echartsResize"
        :change:resize="rdcharts.ecresize"
      />
    </block>
    <block v-else>
      <view
        :id="`UC${cid}`" :prop="uchartsOpts" :change:prop="rdcharts.ucinit" @tap="rdcharts.tap"
        @mousemove="rdcharts.mouseMove" @mousedown="rdcharts.mouseDown" @mouseup="rdcharts.mouseUp"
        @touchstart="rdcharts.touchStart" @touchmove="rdcharts.touchMove" @touchend="rdcharts.touchEnd"
      >
        <canvas
          v-show="showchart" :id="cid" :canvasId="cid"
          :style="{ width: `${cWidth}px`, height: `${cHeight}px`, background }" :disable-scroll="disableScroll"
          @error="_error"
        />
      </view>
    </block>
    ...
    <block v-if="ontouch">
      <canvas
        v-show="showchart" :id="cid" :canvasId="cid" :width="cWidth * pixel" :height="cHeight * pixel"
        :style="{ width: `${cWidth}px`, height: `${cHeight}px`, background }" :disable-scroll="disScroll" @tap="_tap"
        @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" @error="_error"
      />
    </block>
    <block v-if="!ontouch">
      <canvas
    ...
    <block v-if="type2d">
      <view v-if="ontouch" @tap="_tap">
        <canvas
          v-show="showchart" :id="cid" :canvasId="cid"
          :style="{ width: `${cWidth}px`, height: `${cHeight}px`, background }" type="2d" :disable-scroll="disScroll"
          @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" @error="_error"
        />
      </view>
      <view v-if="!ontouch" @tap="_tap">
    ...
    <block v-if="!type2d">
      <view v-if="ontouch" @tap="_tap">
        <canvas
          v-if="showchart" :id="cid" :canvasId="cid"
          :style="{ width: `${cWidth}px`, height: `${cHeight}px`, background }" :disable-scroll="disScroll"
          @touchstart="_touchStart" @touchmove="_touchMove" @touchend="_touchEnd" @error="_error"
        />
      </view>
      <view v-if="!ontouch">
